...
export const Middle = () => {
	...
	// 						let additionalPrice = [
	// 							...products.map((item) => item.ingredientsPrice),
	// 							...drinks
	// 								.filter((drink) => item.productId === drink.productRelationNumber)
	// 								.map((rawDrink) => rawDrink.productPrice),
	// 						].reduce((a, b) => a + b, 0);
	// 						cartItems = [
	// 							...cartItems,
	// 							{
	...
	// 									(drink) => item.productId === drink.productRelationNumber,
	// 								),
	...
	// 								rawDrinks: drinks
	// 									.filter((drink) => item.productId === drink.productRelationNumber)
	// 									.map((rawDrink) => rawDrink.productName),
	// 								rawIngredients: [
	// 									...item.ingredients.map(
	// 										(rawIngredient) => rawIngredient.ingredientName,
	// 									),
	// 								],
	// 							},
	// 						];
	// 					});
	// 					cartItems.forEach(
	// 						(item) => (totalPrice += item.productSubTotal * item.quantity),
	// 					);
	// 					cartDataHandler.dispatch(
	// 						cartDataHandler.handleAddToCart({
	// 							totalPrice,
	// 							data: cartItems,
	// 							apiData: {
	...
	// 					setLoaded(true);
	// 					setErrorMessage(
	// 						'Ha habido un error al cargar los datos del carrito. El carrito está vacío.',
	// 					);
	// 				}
	// 			})
	// 			.catch(() => {
	// 				setErrorMessage(
	// 					'Lo sentimos, ha habido un error al cargar los datos. Refresque la página.',
	// 				);
	// 				setLoaded(true);
	// 				return;
	// 			});
	...
	return (
		<>
			<div className="h-full border-t border-b border-gray overflow-y-auto">
				{
					/* {loaded || (
					<p className="text-center">Estamos cargando los datos, aguarde un momento...</p>
				)}
				{loaded && <p className="text-center">{errorMessage}</p>}
				{loaded && */
					cartState?.map((product) => (
						<CartProduct key={product.id} id={product.id} product={product} />
					))
				}
			</div>
		</>
